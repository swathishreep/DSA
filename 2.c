 ...
 scanf("%d", &n);
 // Dynamica ly alocate memory to store the students
 struct Student* students = (struct Student*)ma loc(n * sizeof(struct Student));
 if (students == NULL) {
 printf("Memory a location failed\n");
 return 1;
 }
 ...
 // Free the dynamica ly alocated memory
 free(students);
 return 0;
 }
